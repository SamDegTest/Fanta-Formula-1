...
const CoppaPage = () => {
  ...
  const groupDivisions = useMemo(() => {
    ...
    const divisions = [];
    // Max number of groups is numParticipants / 2, so each group has at least 2 participants.
    ...
    <div className="animate-fade-in">
      ...
      {isLoadingCups ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {cups.map((cup) => (
            <Link href={`/coppa/${cup.id.replace('cup:', '')}`} key={cup.id} className="block bg-[#1C2541]/80 border border-slate-700/50 rounded-none shadow-lg hover:border-[#F5A623]/50 transition-colors group">
            <div className="flex flex-col justify-between h-full">
              ...
              <div className="p-6 pt-0">
                <div className="border-t border-slate-700/30 flex justify-end items-center gap-2 pt-4">
                    <button
                        onClick={(e) => { e.stopPropagation(); e.preventDefault(); handleManage(cup); }}
                    ...
                    <button
                        onClick={(e) => { e.stopPropagation(); e.preventDefault(); handleDelete(cup.id); }}
